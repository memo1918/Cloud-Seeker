...
export class TimeUnitCategorisation implements UnitCategorisation {
  ...
  getConversionFactor(target: UnitCategorisation): number {
    ...
    if (!(target instanceof TimeUnitCategorisation)) throw new Error(`Incompatible units ${this.unitName} and ${target.unitName}`);

    let targetkey = TimeUnitCategorisation.getUnit(target.selected) || "";
    let targetvalue = TimeUnitCategorisation.conversion[targetkey];

    return targetvalue / this.milliseconds;
  }
