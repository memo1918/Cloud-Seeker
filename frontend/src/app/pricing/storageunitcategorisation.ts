...
export class StorageUnitCategorisation implements UnitCategorisation {
    ...
    getConversionFactor(target: UnitCategorisation): number {
        if (!(target instanceof StorageUnitCategorisation)) throw new Error(`Incompatible units ${this.unitName} and ${target.unitName}`);


        let targetKey = StorageUnitCategorisation.parse(target.selected);
        let targetValue = StorageUnitCategorisation.units[targetKey][0];
